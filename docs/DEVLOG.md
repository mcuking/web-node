...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **836 通过 / 2 预存 skip（77 files）** · `vite build` 绿（worker **2267.91KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M73）候选**：① 把 `internal/errors` 的差分推到**堆栈保真**（`Error.captureStackTrace`/`prepareStackTrace`）；② 扫描器点出的**类原型成员**（`zlib.BrotliCompress.prototype`/`crypto.X509Certificate.prototype` 等）继续对齐；③ `tls`（最后一块代理式纯桩，倾向保留）；④ worker 可选真并行 / 剖析·`resourceLimits` 真替身。
