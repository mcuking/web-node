...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **649/649 通过（54 files）** · `vite build` 绿（worker **2141.15KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **`tty` 换真源码**：✅ **已处理（2026-09-21，M55）**：真 `lib/tty.js` + `lib/internal/tty.js` 已 vendor（MANIFEST 151 → 153），`tty_wrap` binding 让 `isatty` 与颜色深度成为真实现，`ReadStream`/`WriteStream` 构造即抛（标签页没有 TTY 句柄）。`unsupported` 现在只剩 `tls` 和 `vm`。剩：非 tty fd 构造流的错误码与真 Node（`ERR_TTY_INIT_FAILED`）不同，已在 DEVLOG 记录。
   - **下一步（M56）候选**：`vm`（把现有只实现 `runInNewContext` 的 shim 扩成真 `lib/vm.js` 的可及部分：`Script`/`createContext`/`runInContext`）；或继续拓宽 `internal/errors` 的错误码表；或给 `worker_threads` 补上 `MessagePort` 之外的 `Worker` 本体。
