...
| M39 | **npm 深化**：`overrides`/`resolutions`（扁平 / 嵌套 / `.` / `$ref` 四种形态，最长路径优先）、`file:`/`link:` 本地说明符（VFS 目录拷贝 / 本地 `.tgz` 解包）、**有界并发下载**（默认 8，且改成一欠下载全部拉完再写树，失败不留半成品）| ✅ 完成 |
