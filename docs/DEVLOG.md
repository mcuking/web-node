...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **770 通过 / 2 预存 skip（67 files）** · `vite build` 绿（worker **2237.94KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M63）候选**：把 `internal/errors` 的**函数型消息全量差分**再推一格（不只 31 个我们已定义的码，而是用自动参数生成覆盖 Node 全部 316 个码，报出更多可修的偏差）；或 `tls`（最后一块纯桩，倾向保留）；或补一个未覆盖内建模块（`repl`/`node:test` 都因依赖 `internal/modules/*`、`internal/source_map/*` 而代价高）；或 worker 可选真并行 / 剖析·`resourceLimits` 真替身。
