...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **798 通过 / 2 预存 skip（71 files）** · `vite build` 绿（worker **2255.42KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M67）候选**：继续拿扫描器推「公开表面缺口」——剩下较大的是 `http`（12：`Agent`/`OutgoingMessage`/`globalAgent`/`WebSocket`/`validateHeaderName|Value`/`maxHeaderSize`，依赖未 vendor 的 `_http_*` 树，代价高）与 `process` 的 19 个下划线内部量；或 `tls`（最后一块纯桩，倾向保留）；或 worker 可选真并行 / 剖析·`resourceLimits` 真替身；或转向语义深度（而非表面）：选一个已 vendor 的子系统与真 Node 做更细的差分。
