...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **823 通过 / 2 预存 skip（76 files）** · `vite build` 绿（worker **2265.66KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M72）候选**：① 继续把扫描器点出的**函数 arity** 对齐（`http`/`net`/`dns`/`zlib`/`process` 的一批 `0!=N`，纯机械但可观测）；② 把 `internal/errors` 的差分推到**堆栈保真**（`Error.captureStackTrace`/`prepareStackTrace`）；③ `tls`（最后一块代理式纯桩，倾向保留）；④ worker 可选真并行 / 剖析·`resourceLimits` 真替身。
