...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **640/640 通过（53 files）** · `vite build` 绿（worker **2113.99KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M53）候选**：把 `internal/fs/cp/cp.js` 的异步目录拷贝（`CpDirJob`）换成真源驱动；或继续拓宽 `internal/errors` 的错误码表。
