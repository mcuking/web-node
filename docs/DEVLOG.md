...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M60）候选**：`tls`（最后一块纯抛错桩；vendored 无模块 require 它，硬做只能造假，倾向保留）；或继续拓宽 `internal/errors` 的其它表（`SYSTEM_ERROR_CODES`/`CUSTOM_PROPS`）；或 worker 的可选真并行（嵌套 Web Worker）；或把 worker 剖析/`resourceLimits` 的真替身。
