...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **308/308 通过（35 files）** · `vite build` 绿（worker ~1145KB / index ~10.8KB / css ~4.1KB）
...
8. **把 vendored 源改为按需加载**：`vendored.ts` 现在是 eager `import.meta.glob`，每个 vendor 文件都进 bundle（M29 后 worker 达 **~1145KB**，其中 inspect 100KB、util 20KB、assert 家族 ~60KB、event_target ~40KB、abort_controller ~15KB、console+debuglog+readline ~60KB、os ~8KB、timers+timers/promises ~40KB）；若在意体积，可改成按文件 code-split。
