...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **300/300 通过（34 files）** · `vite build` 绿（worker ~1104KB / index ~10.8KB / css ~4.1KB）
...
2j. ~~**`console` 换真源码**~~ ✅ **已解决（2026-09-20，M27）**：真 `lib/console.js` + `internal/console/*` + `internal/cli_table` + `internal/trace_events` + 真 `internal/util/debuglog`；删手写 `builtins/console.ts`；`process.stdout`/`stderr` 改真 `Writable`（lazy getter+setter），新增 `trace_events` binding。剩：`internal/readline/*` 仅 TTY 可达（已 vendor，暂未验证）。
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **`lib/timers.js` + `internal/timers.js`**：真 `Timeout`/`Immediate` 语义（ref/unref/refresh/`promises`），需实现 `timers` binding 的 `timerListMap`/`insert` 等 C++ 表面，中等体量。
   - **`internal/worker/io.js`（MessagePort/MessageChannel）**：真 `AbortController` 的 `[kTransferList]` 已预留接口（现在 lazy require，未注册），接上后 `structuredClone`/`postMessage` 才能真转移。
...
8. **把 vendored 源改为按需加载**：`vendored.ts` 现在是 eager `import.meta.glob`，每个 vendor 文件都进 bundle（M28 后 worker 达 **~1104KB**，其中 inspect 100KB、util 20KB、assert 家族 ~60KB、event_target ~40KB、abort_controller ~15KB、console+debuglog+readline ~60KB、os ~8KB）；若在意体积，可改成按文件 code-split。
