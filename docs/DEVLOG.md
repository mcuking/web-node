...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **803 通过 / 2 预存 skip（72 files）** · `vite build` 绿（worker **2258.11KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M68）候选**：扫描器排出的公开面缺口现在只剩 `process` 的 19 个下划线内部量（`_getActiveHandles`/`_tickCallback`/`dlopen` 等，非公开）；要么补齐它们，要么**转向语义深度**：选一个已 vendor 的子系统与真 Node 做更细的差分（如 `stream`/`fs` 的边界行为），或 `tls`（最后一块纯桩，倾向保留），或 worker 可选真并行 / 剖析·`resourceLimits` 真替身。
