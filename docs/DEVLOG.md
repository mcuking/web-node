...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **780 通过 / 2 预存 skip（69 files）** · `vite build` 绿（worker **2244.46KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M65）候选**：继续拿扫描器推「公开表面缺口」——最大三块是 `crypto` 的类导出（`Cipheriv`/`Decipheriv`/`Hash`/`Hmac`/`KeyObject`/`Sign`/`Verify`/`subtle` 等 21 个，但需真的 WebCrypto 桥）、`http` 的 `Agent`/`OutgoingMessage`/`globalAgent`/`WebSocket`（12 个，依赖 `_http_*` 未 vendor 树）、`net` 的 `SocketAddress`/`BlockList`/`getDefaultAutoSelectFamily*`（10 个）；或 `tls`（最后一块纯桩，倾向保留）；或 worker 可选真并行 / 剖析·`resourceLimits` 真替身；或把 `process` 的 19 个下划线内部量补齐。
