...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **817 通过 / 2 预存 skip（74 files）** · `vite build` 绿（worker **2265.53KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M70）候选**：语义深度继续——① 把**扁平化原型链差分**推广到全部模块（`net.Socket` 目前只继承 EventEmitter、缺整套 stream 方法，考虑改真 `Duplex`）；② 把 `internal/errors` 的差分推到**堆栈保真**（`Error.captureStackTrace`/`prepareStackTrace`）；③ `tls`（最后一块纯桩，倾向保留）；④ worker 可选真并行 / 剖析·`resourceLimits` 真替身。
