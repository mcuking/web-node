...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M55）候选**：`tty`（真 `lib/tty.js` + `tty_wrap`；CLI 的颜色探测靠它）；或继续拓宽 `internal/errors` 的错误码表；或给 `worker_threads` 补上 `MessagePort` 之外的 `Worker` 本体。
