...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **819 通过 / 2 预存 skip（75 files）** · `vite build` 绿（worker **2265.08KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M71）候选**：① 继续把**扁平化原型链差分**跑遍全部模块（已收 `http`/`net`，下一步看 `dns.Resolver`、`crypto.KeyObject`/`X509Certificate` 等“存在但抛错”类的命名对齐）；② 把 `internal/errors` 的差分推到**堆栈保真**（`Error.captureStackTrace`/`prepareStackTrace`）；③ `tls`（最后一块纯桩，倾向保留）；④ worker 可选真并行 / 剖析·`resourceLimits` 真替身。
