...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **810 通过 / 2 预存 skip（73 files）** · `vite build` 绿（worker **2260.68KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M69）候选**：公开模块表面已基本收齐（最后一个缺口 `process` 也清了），**转向语义深度**：① 把 `internal/errors` 的差分推到**全量函数型消息 + 堆栈保真**（`Error.captureStackTrace`/`prepareStackTrace`）；② 选一个已 vendor 子系统（如 `stream`/`fs`/`net`）与真 Node 做更细的边界差分；③ `tls`（最后一块纯桩，倾向保留）；④ worker 可选真并行 / 剖析·`resourceLimits` 真替身。
