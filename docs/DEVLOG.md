...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **249/249 通过** · `vite build` 绿（worker ~653KB / index ~10.8KB / css ~4.1KB）
...
8. **把 vendored 源改为按需加载**：`vendored.ts` 现在是 eager `import.meta.glob`，每个 vendor 文件都进 bundle（M24 后 worker 达 **~943KB**，其中 inspect 100KB、util 20KB、assert 家族 ~60KB）；若在意体积，可改成按文件 code-split。
