...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **772 通过 / 2 预存 skip（68 files）** · `vite build` 绿（worker **2238.47KB** / index ~10.84KB / css ~4.06KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M64）候选**：`tls`（最后一块纯桩，倾向保留）；或补一个未覆盖内建模块（`repl`/`node:test` 都因依赖 `internal/modules/*`、`internal/source_map/*` 而代价高）；或 worker 可选真并行 / 剖析·`resourceLimits` 真替身；或把 `CUSTOM_PROPS`/错误堆栈（`Error.captureStackTrace` 保真）再推一格。
