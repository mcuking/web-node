...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **279/279 通过（31 files）** · `vite build` 绿（worker ~1018KB / index ~10.8KB / css ~4.1KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **`internal/abort_controller.js`**：目前仍是 shim，跟 `EventTarget` 一起换真源码的时机到了；它只需再 vendor `internal/worker/js_transferable` / `internal/worker/io`（或对这两者做小型 shim），就能拿真 `AbortController`/`AbortSignal`。
...
8. **把 vendored 源改为按需加载**：`vendored.ts` 现在是 eager `import.meta.glob`，每个 vendor 文件都进 bundle（M25 后 worker 达 **~1018KB**，其中 inspect 100KB、util 20KB、assert 家族 ~60KB、event_target ~40KB）；若在意体积，可改成按文件 code-split。
