...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M62）候选**：`tls`（最后一块纯桩，倾向保留）；或继续拓宽 `internal/errors`（`CUSTOM_PROPS`/函数型消息的全量差分）；或 worker 可选真并行 / 剖析·`resourceLimits` 真替身；或某个未覆盖内建模块。
