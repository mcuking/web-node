...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **下一步（M54）候选**：`v8` 模块（`serialize`/`deserialize` 的 V8 序列化格式、`getHeapStatistics`）；或继续拓宽 `internal/errors` 的错误码表；或 `querystring` 之外的遗留小模块。
