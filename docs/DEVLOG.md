...
**质量门禁**：`tsc --noEmit` 干净 · `vitest run` **293/293 通过（33 files）** · `vite build` 绿（worker ~1096KB / index ~10.8KB / css ~4.1KB）
...
3. **继续 vendoring（按 ROI 排序）**：
   ...
   - **`lib/os.js`**：小的候选（需 `constants`/`credentials`/`os` binding），可换真源码。
...
8. **把 vendored 源改为按需加载**：`vendored.ts` 现在是 eager `import.meta.glob`，每个 vendor 文件都进 bundle（M27 后 worker 达 **~1096KB**，其中 inspect 100KB、util 20KB、assert 家族 ~60KB、event_target ~40KB、abort_controller ~15KB、console+debuglog+readline ~60KB）；若在意体积，可改成按文件 code-split。
