...
/**
 * M92.2: `crypto.diffieHellman({ privateKey, publicKey })`.
 * `tools/crypto-dh-secret-probe.cjs` runs unchanged on real Node and inside
 * web-node; the observation must match `test/fixtures/crypto-dh-secret.json`.
 */
...
describe('crypto.diffieHellman (differential vs real Node)', () => {
  it('matches the oracle exactly', async () => {
    const program = fs.readFileSync('tools/crypto-dh-secret-probe.cjs', 'utf8');
    const expected = JSON.parse(fs.readFileSync('test/fixtures/crypto-dh-secret.json', 'utf8'));
