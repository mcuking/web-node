...
/**
 * M93.3: AES-CCM. `tools/crypto-ccm-probe.cjs` runs unchanged on real Node and
 * inside web-node; the observation must match `test/fixtures/crypto-ccm.json`
 * (recorded from real Node via `tools/crypto-ccm-oracle.mjs`).
 */
...
describe('AES-CCM (differential vs real Node)', () => {
  it('matches the oracle exactly', async () => {
    const program = fs.readFileSync('tools/crypto-ccm-probe.cjs', 'utf8');
    const expected = JSON.parse(fs.readFileSync('test/fixtures/crypto-ccm.json', 'utf8'));
