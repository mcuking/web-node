...
/**
 * M93.2: the DES / 3DES family. `tools/crypto-des-probe.cjs` runs unchanged on
 * real Node and inside web-node; the observation must match
 * `test/fixtures/crypto-des.json` (recorded from real Node via
 * `tools/crypto-des-oracle.mjs`).
 */
...
describe('DES / 3DES (differential vs real Node)', () => {
  it('matches the oracle exactly', async () => {
    const program = fs.readFileSync('tools/crypto-des-probe.cjs', 'utf8');
    const expected = JSON.parse(fs.readFileSync('test/fixtures/crypto-des.json', 'utf8'));
