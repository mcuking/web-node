...
export function transformEsmToCjs(source: string, moduleUrl: string): TransformResult {
  ...
  // async loader. `import.meta` must be rewritten first: afterwards no `import`
  // keyword remains except real dynamic imports.
  const body = (header + out.join('\n') + '\n' + exportLines.join('\n'))
    .replace(/\bimport\.meta\.url\b/g, '__mod_url')
    .replace(/\bimport\.meta\.filename\b/g, '__mod_file')
    .replace(/\bimport\.meta\.dirname\b/g, '__mod_dir')
    .replace(/\bimport\.meta\b/g, '({ url: __mod_url })')
    .replace(/(?<![\w$.])import\s*\(/g, `${IMPORT_BINDING}(`);
