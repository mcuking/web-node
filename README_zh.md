...
与 Node 一致；缺的是并行能力（文档里写明），而需要原生线程的东西（`eval`、worker 标准 IO、
`resourceLimits`、剖析、嵌套 worker）一律抛错。
